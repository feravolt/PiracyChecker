 ...
 * with the {@link Policy}, the library calls the appropriate callback method to communicate the
 * result. <p> <b>The callback does not occur in the original checking thread.</b> Your application
 * should post to the appropriate handling thread or lock accordingly. <p> The reason that is passed
 * back with allow/dontAllow is the base status handed to the policy for allowed/disallowing the
 * license. Policy.RETRY will call allow or dontAllow depending on other statistics associated with
...
public interface LibraryCheckerCallback {
     ...
     *
     * @param reason Policy.LICENSED or Policy.RETRY typically. (although in theory the policy can return
    ...
    /**
     * Error in application code. Caller did not call or set up license checker correctly. Should be
     * considered fatal.
     */
